...
const ManagerCourseInstructor: React.FC = () => {
  ...
  const [fileList, setFileList] = useState<any[]>([]); // State để lưu danh sách file đã chọn
      ...
      .then(values => {
        ...
        if (selectedLesson) {
          ...
          const newLesson = {
            ...
            videoUrl: fileList.length > 0 ? URL.createObjectURL(fileList[0]) : 'https://www.example.com/video.mp4', // Thay đổi videoUrl để lưu trữ hoặc sử dụng file đã chọn
          };
          setLessons([...lessons, newLesson]);
        }
        setLessonModalVisible(false);
        setSelectedLesson(null);
        setFileList([]); // Xóa danh sách file đã chọn sau khi lưu
      })
      .catch(info => {
        console.log('Validate Failed:', info);
      });
  ...
  // Hàm xử lý sự kiện cho nút "Add"
  const handleAddToLesson = (lessonKey: string) => {
    // Logic để thêm vào lesson, ví dụ mở modal hoặc thực hiện hành động khác
    console.log(`Add to lesson ${lessonKey}`);
  };
    ...
    },
    {
      ...
      render: (text: string, record: DataType) => (
        <div style={{ textAlign: 'center' }}>
          ...
          <Button icon={<DeleteOutlined />} className="text-white bg-red-600" onClick={() => handleDelete(record)}></Button>
        </div>
      ),
    },
  ];
  ...
  return (
    <Layout style={{ minHeight: '100vh' }}>
                        ...
                        <SubMenu key="sub1" title="Session" icon={<FileOutlined />}>
                          {lessons.map(lesson => (
                            <Menu.Item key={lesson.key} icon={lesson.preview ? <FileOutlined /> : <LockOutlined />} className="flex items-center justify-between h-24">
                              ...
                              <div>
                                <Button icon={<UploadOutlined />} className="mr-2" onClick={() => handleEditLesson(lesson)}></Button>
                                <Button icon={<DeleteOutlined />} className="mr-2 text-white bg-red-600" onClick={() => handleLessonDelete(lesson.key)}></Button>
                                <Button icon={<PlusOutlined />} className="text-white bg-green-600" onClick={() => handleAddToLesson(lesson.key)}>Add</Button>
                              </div>
                            </Menu.Item>
                          ))}
                          <Menu.Item>
      ...
      <Modal
        ...
        onCancel={() => {
          setLessonModalVisible(false);
          setFileList([]); // Xóa danh sách file đã chọn nếu hủy
        }}
        ...
        <Form form={form} layout="vertical" name="form_in_modal">
          ...
          >
            <Upload
              accept="video/*"
              beforeUpload={(file) => {
                setFileList([file]); // Lưu file đã chọn vào state fileList
                return false; // Chặn Ant Design Upload tự động upload
              }}
              fileList={fileList}
              onRemove={() => setFileList([])} // Xóa file đã chọn khi bấm nút xóa trong Upload
            >
